...
- 2026-08-26: Falling 진입 순간 빙판 위 runtime gameplay Actor를 RayTrace로 감지하고, Falling 동안 Piece 하강 Delta만큼 함께 이동시키는 Carry 구조를 C++에 추가했다. C4800, C4458 컴파일 오류 보정은 완료했으며, 보정 후 재빌드와 PIE 검증은 아직 확인 필요 상태다.
...
- 최종 Camera Shake Scale은 `WarningCameraShakeScale * Lerp(0.3, 1.0, Clamp(WarningAlpha, 0.0f, 1.0f)^2)` 흐름으로 계산한다.
...
- [확인 필요] Carry 컴파일 오류 보정 후 C++ 재빌드 성공
- [확인 필요] UHT 검증 완료
...
- Warning Camera Shake 기본 동작과 WarningAlpha 강도 연동은 C++ 구현과 PIE 동작 확인을 완료했다.
...
- [ ] [확인 필요] PIE에서 GiftBox, Campfire, Ground Snowball이 Falling Piece와 함께 하강한다.
- [ ] [확인 필요] PIE에서 Held, Rolling, Thrown Snowball과 Character/Pawn은 Carry 대상에서 제외된다.
