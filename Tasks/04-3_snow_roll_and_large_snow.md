...
- 눈덩이 Sweep이 장애물에 막히면 서버는 눈덩이 중심에서 캐릭터 위치로 향하는 수평 방향으로 굴리는 캐릭터를 즉시 Sweep 이동시킨다.
...
- [x] E+WASD 굴리기 중 눈덩이가 장애물을 통과하지 않고 그 앞에서 멈추며, 전진 입력을 유지하면 굴리는 플레이어가 눈덩이 중심에서 멀어지는 방향으로 조금씩 밀려나는지 확인한다.
- [x] 플레이어 캡슐이 장애물에 닿기 전에도 눈덩이가 먼저 장애물에 닿는 순간부터 밀림이 시작되는지 확인한다.
- [x] 벽을 정면과 비스듬한 각도에서 밀었을 때 모두 눈덩이와 플레이어를 잇는 반대 방향으로 밀리고 위로 튀지 않는지 확인한다.
- [x] 첫 이동 이후에도 굴리는 플레이어 자신의 캡슐에 막히지 않고 WASD 이동을 계속 따라오는지 확인한다.
- [x] 캐릭터 캡슐도 굴리는 눈덩이에 막히지 않아 서버 이동속도와 굴리기 방향이 계속 유지되는지 확인한다.
- [x] 첫 성장 직후 커진 충돌 구체가 바닥에 파고들어 다음 Sweep이 멈추지 않는지 확인한다.
...
- [ ] 큰 눈을 획득하기 전에 스프린트 중이었다면 즉시 스프린트가 해제되는지 확인한다.
- [ ] 큰 눈 보유 중 Shift를 눌러도 스프린트가 시작되지 않는지 확인한다.
- [ ] 화면상 최대 크기에 도달한 성장값이 미세한 오차로 `1`보다 작더라도 큰 눈 운반속도와 스프린트 차단이 적용되는지 확인한다.
- [ ] 큰 눈을 내려놓으면 일반 걷기와 스프린트가 다시 가능한지 확인한다.
