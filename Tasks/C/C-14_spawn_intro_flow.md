...
- [x] 서버가 같은 팀원이 이미 스폰된 경우 팀원 옆의 시작 지점을 우선 선택한다.
...
- 2026-08-24: PvP 진입 시 `ASnowRumbleGameMode`가 새 플레이어의 팀과 이미 스폰된 캐릭터의 팀을 비교해 같은 팀원이 있으면 기본 반경 180cm 안의 충돌 없는 위치를 우선 사용하게 했다. 같은 팀원이 없거나 후보를 찾지 못하면 기존 PlayerStart 분산 스폰으로 fallback한다.
