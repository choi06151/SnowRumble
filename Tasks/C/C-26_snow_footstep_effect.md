...
- [x] 눈 표면과 일반 바닥을 구분해 `SnowFootstepEffect` 또는 `NormalFootstepEffect` Niagara를 발밑 위치에 직접 스폰하는 C++ 슬롯 제공
...
- 생성·변경 파일: `Tasks/C/C-26_snow_footstep_effect.md`, `Tasks/C/PLAN_C.md`, `docs/PLANS.md`, `Source/SnowRumble/Player/SnowRumbleCharacter.h`, `Source/SnowRumble/Player/SnowRumbleCharacter.cpp`, `Content/Characters/BP_SnowRumbleCharacter.uasset`, `Content/Characters/BP_SnowmanCharacter_K.uasset`, `Content/Maps/L_IceGlacier_J.umap`, `Content/Maps/L_Lobby.umap`, `Content/Models/GiftBox/MI_GiftBox.uasset`, `Content/Models/M_MasterMaterial.uasset`, `Content/VFX/NS_WeaponBurst.uasset`, `Content/A_Surface_Footstep/`
...
- 제공할 Blueprint 슬롯: `SnowFootstepEffect`, `NormalFootstepEffect`, `FootstepEffectSurfaceOffset`
...
- 2026-08-28: 눈/일반 바닥 발걸음 Niagara 슬롯을 추가하고, `SnowSurface` 여부에 따라 C++가 발밑 위치에서 `SnowFootstepEffect` 또는 `NormalFootstepEffect`를 스폰하도록 보강했다. 사용자 확인 요청에 따라 관련 자산 연결과 수동 결과 확인을 완료 작업으로 게시한다.
...
- `BP_SnowRumbleCharacter`에서 눈 표면용 `SnowFootstepEffect`, 일반 바닥용 `NormalFootstepEffect`, 필요 시 `FootstepEffectSurfaceOffset`을 지정한다.
...
- [x] PIE 또는 Standalone에서 눈 표면 위를 걸을 때 발 착지 시점에만 `OnSnowFootstepEffect` 표현이 재생됨
- [x] `SnowSurface` 태그가 없는 바닥에서는 같은 AnimNotify가 호출되어도 눈 밟힘 표현이 재생되지 않음
- [x] Listen Server 호스트와 클라이언트 화면에서 각자 보이는 캐릭터의 눈 밟힘 표현이 과도하게 중복되거나 누락되지 않음
- [x] `FootstepSound`가 발 socket 아래의 `SnowSurface` 위치에서 attenuation과 함께 재생됨
- [x] 일반 바닥에서 `NormalFootstepSound`가 재생되고, 눈 표면 VFX/stamp는 실행되지 않음
