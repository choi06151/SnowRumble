...
- [x] 로비에서는 캐릭터가 피해를 받지 않게 한다.
- [x] 기존 `OverheadTimedActionWidget`이 모든 클라이언트 화면에서 얼음 상태의 60초 사망 타이머를 1에서 0으로 표시하게 한다.
...
- 변경 파일: `Source/SnowRumble/Player/SnowRumbleCharacter.*`, `Source/SnowRumble/UI/OverheadTimedActionWidget.*`, `Source/SnowRumble/UI/SpectatorWidget_C.*`, `Tasks/C/C-06_freeze_death_spectate.md`, `Tasks/C/PLAN_C.md`, `docs/PLANS.md`
...
- 제공할 계약:
  ...
  - `UOverheadTimedActionWidget::SetObservedCharacter()`: 원격 캐릭터의 얼음 진행도를 기존 진행 바에 연결한다.
  - `ASnowRumbleCharacter::SpectatorWidgetClass`: 얼음·사망 상태에서 생성할 관전 WBP 슬롯이다.
  - `USpectatorWidget::SetSpectatorViewTarget(ASnowRumbleCharacter*)`: 로컬 관전 대상과 `CurrentViewTargetIdText`에 표시할 대상 닉네임을 갱신한다.
  - `USpectatorWidget::GetCurrentViewTargetId()`: 기존 호환용으로 현재 관전 대상의 `APlayerState::GetPlayerId()` 값을 반환한다.
...
- 얼음·사망 상태에서 `SpectatorWidgetClass`에 지정한 WBP를 표시하고, `CurrentViewTargetIdText` TextBlock에 현재 관전 대상의 닉네임을 표시한다.

## 수동 작업

- 플레이어 캐릭터 Blueprint의 `OverheadTimedActionWidgetClass`에 기존 `WBP_OverheadTimedActionWidget`을 지정한다. WBP 내부의 `TimedActionProgressBar` 바인딩은 그대로 사용한다.
...
- 관전 WBP는 `USpectatorWidget`을 부모로 지정하고 `CurrentViewTargetIdText` 이름의 TextBlock을 배치한 뒤, 플레이어 Blueprint의 `SpectatorWidgetClass`에 연결한다. TextBlock에는 대상 PlayerState의 닉네임이 자동 표시된다.
...
- [x] 관전 카메라 갱신 빈도와 로컬 위치·회전·FOV 보간을 높여 끊김을 완화한다.
- [ ] 팀 전멸 우선순위 점검 완료
- [ ] S-07 인계 완료
...
- [ ] 얼음 상태에서 기존 `OverheadTimedActionWidget` ProgressBar가 모든 플레이어 화면에서 1에서 0으로 60초 동안 감소한다.
