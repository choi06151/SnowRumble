...
void UOverheadNameplateWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime) {
	...
	if (CurrentDisplayName != LastDisplayName || CurrentTeamColor != LastTeamColor ||
		bCurrentShowReadyImage != bLastShowReadyImage || bCurrentShowHostImage != bLastShowHostImage) {
		RefreshNameplatePresentation();
	}
}

void UOverheadNameplateWidget::SetObservedCharacter(ASnowRumbleCharacter* NewCharacter) {
...
FString UOverheadNameplateWidget::GetDisplayName() const {
	return ObservedCharacter ? ObservedCharacter->GetOverheadPlayerName() : FString();
}

FLinearColor UOverheadNameplateWidget::GetTeamColor() const {
	return ObservedCharacter ? ObservedCharacter->GetOverheadTeamColor() : FLinearColor::White;
}
...
bool UOverheadNameplateWidget::ShouldShowReadyImage() const {
	...
	const ASnowRumblePlayerState* PlayerState =
		ObservedCharacter ? ObservedCharacter->GetPlayerState<ASnowRumblePlayerState>() : nullptr;
	return PlayerState && !PlayerState->IsLobbyHost() && PlayerState->IsLobbyReady();
}
...
void UOverheadNameplateWidget::RefreshNameplatePresentation() {
	...
	if (ReadyStateImage) {
		ReadyStateImage->SetVisibility(bLastShowReadyImage ? ESlateVisibility::SelfHitTestInvisible
														   : ESlateVisibility::Collapsed);
	}

	if (HostStateImage) {
		HostStateImage->SetVisibility(bLastShowHostImage ? ESlateVisibility::SelfHitTestInvisible
														 : ESlateVisibility::Collapsed);
	}
}
