...
void UVoiceMutePlayerRowWidget::SetVoiceMutePlayer(ASnowRumblePlayerState* NewPlayerState,
...
ASnowRumblePlayerState* UVoiceMutePlayerRowWidget::GetObservedPlayerState() const {
	return ObservedPlayerState;
}

void UVoiceMutePlayerRowWidget::RefreshRow() {
	const bool bIsMuted = VoicePlayerController && ObservedPlayerState &&
						  VoicePlayerController->IsVoicePlayerManuallyMuted(ObservedPlayerState);

	if (PlayerNameText) {
		PlayerNameText->SetText(ObservedPlayerState ? FText::FromString(ObservedPlayerState->GetLobbyPlayerName())
													: FText::GetEmpty());
	}
	if (MuteButtonText) {
		MuteButtonText->SetText(bIsMuted ? NSLOCTEXT("SnowRumble", "VoiceMuteUnmuteButton", "해제")
										 : NSLOCTEXT("SnowRumble", "VoiceMuteMuteButton", "음소거"));
	}
	if (MuteButton) {
		MuteButton->SetIsEnabled(VoicePlayerController != nullptr && ObservedPlayerState != nullptr);
	}
...
void UVoiceMutePlayerRowWidget::BindRowButton() {
	if (MuteButton) {
		MuteButton->OnClicked.AddUniqueDynamic(this, &UVoiceMutePlayerRowWidget::HandleMuteButtonClicked);
	}
}
