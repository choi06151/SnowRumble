...
void UAnimNotify_SnowFootstep::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	...
	ASnowRumbleCharacter* Character = Cast<ASnowRumbleCharacter>(MeshComp->GetOwner());
	if (!Character) {
		return;
	}
...
FString UAnimNotify_SnowFootstep::GetNotifyName_Implementation() const {
	return FootSocketName.IsNone() ? TEXT("Snow Footstep")
