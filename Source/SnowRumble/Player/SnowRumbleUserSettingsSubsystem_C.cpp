...
void USnowRumbleUserSettingsSubsystem::Initialize(FSubsystemCollectionBase& Collection) {
	Super::Initialize(Collection);

	FCoreDelegates::GatherAdditionalLocResPathsCallback.AddUObject(
		this, &USnowRumbleUserSettingsSubsystem::AddSnowRumbleLocalizationPath);
...
void USnowRumbleUserSettingsSubsystem::SetKeyBinding(FName BindingId, FKey NewKey) {
	if (BindingId.IsNone()) {
		return;
	}
...
FKey USnowRumbleUserSettingsSubsystem::GetKeyBinding(FName BindingId, FKey DefaultKey) const {
...
void USnowRumbleUserSettingsSubsystem::SetMouseSensitivity(float NewSensitivity) {
	MouseSensitivity = FMath::Clamp(NewSensitivity, MinMouseSensitivity, MaxMouseSensitivity);
	SaveConfig();
}
...
float USnowRumbleUserSettingsSubsystem::GetMouseSensitivity() const {
	return FMath::Clamp(MouseSensitivity, MinMouseSensitivity, MaxMouseSensitivity);
}
...
void USnowRumbleUserSettingsSubsystem::SetMicrophoneMode(ESnowRumbleMicrophoneMode NewMode) {
	MicrophoneMode = NewMode;
	SaveConfig();
	OnMicrophoneSettingsChanged.Broadcast();
}

ESnowRumbleMicrophoneMode USnowRumbleUserSettingsSubsystem::GetMicrophoneMode() const {
	return MicrophoneMode;
}

ESnowRumbleMicrophoneMode USnowRumbleUserSettingsSubsystem::GetDefaultMicrophoneMode() const {
	return DefaultMicrophoneMode;
}

void USnowRumbleUserSettingsSubsystem::SetMicrophoneDeviceId(const FString& NewDeviceId) {
...
void USnowRumbleUserSettingsSubsystem::SetLanguageCulture(const FString& NewCulture) {
	const FString Culture = NewCulture.Equals(TEXT("en"), ESearchCase::IgnoreCase) ? TEXT("en") : TEXT("ko");
...
void USnowRumbleUserSettingsSubsystem::ApplyLanguageCulture(const FString& Culture) {
	const bool bCultureApplied = FInternationalization::Get().SetCurrentLanguageAndLocale(Culture);

	FTextLocalizationManager::Get().RefreshResources([Culture]() { LogLanguageProbe(Culture); });

	UE_LOG(LogTemp, Log, TEXT("SnowRumble language requested=%s applied=%s currentLanguage=%s currentLocale=%s"),
		   *Culture, bCultureApplied ? TEXT("true") : TEXT("false"),
		   *FInternationalization::Get().GetCurrentLanguage()->GetName(),
		   *FInternationalization::Get().GetCurrentLocale()->GetName());
}

void USnowRumbleUserSettingsSubsystem::AddSnowRumbleLocalizationPath(TArray<FString>& LocalizationPaths) {
	LocalizationPaths.AddUnique(FPaths::ProjectContentDir() / TEXT("Localization/SnowRumble"));
}

void USnowRumbleUserSettingsSubsystem::LogLanguageProbe(const FString& Culture) {
	const FText ProbeText = NSLOCTEXT("", "497C2BE446D905082A79A1863B8734D5", "방 만들기");

	UE_LOG(LogTemp, Log, TEXT("SnowRumble language resources refreshed culture=%s probe='%s'"), *Culture,
		   *ProbeText.ToString());
}
