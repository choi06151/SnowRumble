...
void USnowRumbleCustomizationSubsystem::SetCustomizationData(const FSnowRumbleCustomizationData& NewData) {
	CustomizationData = SanitizeCustomizationData(NewData);
}

FSnowRumbleCustomizationData USnowRumbleCustomizationSubsystem::GetCustomizationData() const {
...
FSnowRumbleCustomizationData USnowRumbleCustomizationSubsystem::GetDefaultCustomizationData() {
...
USnowRumbleCustomizationSubsystem::SanitizeCustomizationData(const FSnowRumbleCustomizationData& Data) {
	...
	SanitizedData.HatMeshIndex = FMath::Clamp(SanitizedData.HatMeshIndex, INDEX_NONE, 255);
	SanitizedData.GlassesMeshIndex = FMath::Clamp(SanitizedData.GlassesMeshIndex, INDEX_NONE, 255);
	SanitizedData.NoseMeshIndex = FMath::Clamp(SanitizedData.NoseMeshIndex, INDEX_NONE, 255);
	SanitizedData.EarmuffsMeshIndex = FMath::Clamp(SanitizedData.EarmuffsMeshIndex, INDEX_NONE, 255);
	...
	if (SanitizedData.PaintStrokes.Num() > MaxPaintStrokeCount) {
		SanitizedData.PaintStrokes.RemoveAt(0, SanitizedData.PaintStrokes.Num() - MaxPaintStrokeCount);
	}
