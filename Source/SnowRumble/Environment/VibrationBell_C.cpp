...
AVibrationBell::AVibrationBell() {
	...
	BellMeshComponent = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("BellMeshComponent"));
...
void AVibrationBell::Tick(float DeltaSeconds) {
	...
	const float Angle = Frequency > 0.0f
							? FMath::Sin(ShakeElapsedSeconds * Frequency * UE_TWO_PI) * ShakeRotationAmplitudeDegrees
							: 0.0f;
	BellMeshComponent->SetRelativeRotation(BellMeshBaseRelativeRotation + FRotator(0.0f, Angle, 0.0f));
}

float AVibrationBell::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator,
								 AActor* DamageCauser) {
	const float AppliedDamage = Super::TakeDamage(DamageAmount, DamageEvent, EventInstigator, DamageCauser);

	if (HasAuthority() && DamageAmount > 0.0f && Cast<ASnowballItem>(DamageCauser)) {
...
void AVibrationBell::MulticastPlayHitFeedback_Implementation() {
	StartShake();
	SnowRumbleAudio::PlaySoundAtLocation(this, HitSound, ESnowRumbleAudioMixChannel::Gameplay, GetActorLocation(), 1.0f,
										 1.0f, HitSoundAttenuation);
}
