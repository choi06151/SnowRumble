...
APhotoInteractionActor::APhotoInteractionActor() {
	...
	PhotoMeshComponent = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("PhotoMeshComponent"));
	...
	PhotoCameraComponent = CreateDefaultSubobject<UCameraComponent>(TEXT("PhotoCameraComponent"));
	PhotoCameraComponent->SetupAttachment(PhotoMeshComponent);
	PhotoCameraComponent->bAutoActivate = true;
	bFindCameraComponentWhenViewTarget = true;
}

bool APhotoInteractionActor::CanInteractWith(const ASnowRumbleCharacter* Character) const {
	...
	const FVector ClosestPoint = PhotoBounds.GetBox().GetClosestPointTo(Character->GetActorLocation());
	return FVector::DistSquared(Character->GetActorLocation(), ClosestPoint) <= FMath::Square(InteractionRadius);
}
...
void APhotoInteractionActor::AddPhotoCameraRotation(float YawDelta, float PitchDelta) {
	if (!PhotoCameraComponent) {
		return;
	}

	FRotator CameraRotation = PhotoCameraComponent->GetRelativeRotation();
	CameraRotation.Yaw = FRotator::NormalizeAxis(CameraRotation.Yaw + YawDelta);
	CameraRotation.Pitch = FMath::Clamp(CameraRotation.Pitch + PitchDelta, -CameraPitchLimit, CameraPitchLimit);
