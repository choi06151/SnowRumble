...
ALobbyInteractionBoard::ALobbyInteractionBoard() {
	...
	BoardMeshComponent = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("BoardMeshComponent"));
	...
	FocusCameraComponent = CreateDefaultSubobject<UCameraComponent>(TEXT("FocusCameraComponent"));
	...
	BoardWidgetComponent = CreateDefaultSubobject<UWidgetComponent>(TEXT("BoardWidgetComponent"));
...
bool ALobbyInteractionBoard::CanInteractWith(const ASnowRumbleCharacter* Character) const {
	...
	return FVector::DistSquared(CharacterLocation, ClosestBoardPoint) <= FMath::Square(InteractionRadius);
}
...
void ALobbyInteractionBoard::HandleBoardAction(ASnowRumbleCharacter* Character, ELobbyBoardAction BoardAction) {
	if (!HasAuthority() || !CanInteractWith(Character)) {
		return;
	}
...
FVector ALobbyInteractionBoard::GetFocusLocation() const {
	return BoardMeshComponent ? BoardMeshComponent->Bounds.Origin : GetActorLocation();
}
...
void ALobbyInteractionBoard::GetBoardWidgetComponents(TArray<UWidgetComponent*>& OutWidgetComponents) const {
	...
	for (UWidgetComponent* WidgetComponent : WidgetComponents) {
		...
		const ULobbyBoardWidget* BoardWidget = Cast<ULobbyBoardWidget>(WidgetComponent->GetUserWidgetObject());
		if (BoardWidget) {
...
void ALobbyInteractionBoard::SetFocusedCharacter(ASnowRumbleCharacter* Character) {
	...
	ALobbyPlayerController* LobbyPlayerController =
		Character ? Cast<ALobbyPlayerController>(Character->GetController()) : nullptr;
	...
	for (UWidgetComponent* WidgetComponent : WidgetComponents) {
		if (WidgetComponent && LobbyPlayerController) {
			WidgetComponent->SetOwnerPlayer(LobbyPlayerController->GetLocalPlayer());
		}

		ULobbyBoardWidget* BoardWidget = Cast<ULobbyBoardWidget>(WidgetComponent->GetUserWidgetObject());
		if (BoardWidget) {
...
void ALobbyInteractionBoard::InitializeBoardWidget() {
	...
	for (UWidgetComponent* WidgetComponent : WidgetComponents) {
		ULobbyBoardWidget* BoardWidget =
			WidgetComponent ? Cast<ULobbyBoardWidget>(WidgetComponent->GetUserWidgetObject()) : nullptr;
		if (BoardWidget) {
