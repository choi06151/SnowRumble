...
void AGrabbablePhysicsObject::ConfigureInteractionSettings(float NewPlayerPushStrength, int32 NewInteractionsToBreak,
...
void AGrabbablePhysicsObject::ConfigureReplicatedVisuals(UStaticMesh* NewStaticMesh,
...
void AGrabbablePhysicsObject::RegisterInteraction() {
	...
	InteractionCount = FMath::Min(InteractionCount + 1, FMath::Max(InteractionsToBreak, 1));
	ForceNetUpdate();
...
void AGrabbablePhysicsObject::MulticastPlayInteractionBreakEffect_Implementation(FVector_NetQuantize Location) {
	if (InteractionBreakEffect) {
		UNiagaraFunctionLibrary::SpawnSystemAtLocation(this, InteractionBreakEffect, Location, GetActorRotation());
	}
}

void AGrabbablePhysicsObject::MulticastPlayInteractionBreakSound_Implementation(FVector_NetQuantize Location) {
	SnowRumbleAudio::PlaySoundAtLocation(this, InteractionBreakSound, ESnowRumbleAudioMixChannel::Gameplay, Location,
										 1.0f, 1.0f, InteractionBreakSoundAttenuation);
}

void AGrabbablePhysicsObject::PushCharacterFromGrabMotion(ACharacter* Character, FVector MotionDirection) {
	...
	if (PushDirection.IsNearlyZero()) {
		PushDirection = (Character->GetActorLocation() - GetActorLocation()).GetSafeNormal2D();
	}
	if (!PushDirection.IsNearlyZero()) {
...
void AGrabbablePhysicsObject::TickGrabbedByCharacter(ACharacter* /*Grabber*/, FVector /*HeldMotion*/,
													 float /*DeltaTime*/) {}

void AGrabbablePhysicsObject::HandleComponentHit(UPrimitiveComponent* HitComponent, AActor* OtherActor,
												 UPrimitiveComponent* OtherComponent, FVector NormalImpulse,
												 const FHitResult& Hit) {
	if (!HasAuthority() || !bIsHeldByCharacter || PlayerPushStrength <= 0.0f || !OtherActor) {
		return;
	}
	...
	const FVector PushDirection =
		PhysicsComponent ? PhysicsComponent->GetPhysicsLinearVelocity().GetSafeNormal2D() : FVector::ZeroVector;
	PushCharacterFromGrabMotion(Character, PushDirection);
}

void AGrabbablePhysicsObject::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {
