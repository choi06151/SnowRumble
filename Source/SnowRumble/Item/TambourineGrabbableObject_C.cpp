...
void ATambourineGrabbableObject::HandleGrabbedByCharacter(ACharacter* Grabber) {
	...
	PreviousGrabberControlRotation = Grabber ? Grabber->GetControlRotation() : FRotator::ZeroRotator;
...
void ATambourineGrabbableObject::TickGrabbedByCharacter(ACharacter* Grabber, FVector HeldMotion, float DeltaTime) {
	Super::TickGrabbedByCharacter(Grabber, HeldMotion, DeltaTime);

	if (!HasAuthority() || !Grabber || !JingleSound || DeltaTime <= SMALL_NUMBER) {
		return;
	}
	...
	const FRotator RotationDelta = (CurrentGrabberControlRotation - PreviousGrabberControlRotation).GetNormalized();
	PreviousGrabberControlRotation = CurrentGrabberControlRotation;

	const float RotationDistanceDegrees =
		FMath::Abs(RotationDelta.Yaw) + FMath::Abs(RotationDelta.Pitch) + FMath::Abs(RotationDelta.Roll);
	const float LookSpeedDegreesPerSecond = RotationDistanceDegrees / DeltaTime;
	if (LookSpeedDegreesPerSecond < MinimumGrabberLookSpeedForJingle) {
		return;
	}

	AccumulatedHeldRotationDegrees += RotationDistanceDegrees;
	if (AccumulatedHeldRotationDegrees < MinimumHeldRotationDegreesForJingle) {
		return;
	}

	AccumulatedHeldRotationDegrees = 0.0f;
	const float PitchMultiplier = FMath::FRandRange(FMath::Min(MinimumJinglePitch, MaximumJinglePitch),
													FMath::Max(MinimumJinglePitch, MaximumJinglePitch));
	MulticastPlayJingle(GetActorLocation(), PitchMultiplier);
}

void ATambourineGrabbableObject::MulticastPlayJingle_Implementation(FVector_NetQuantize Location,
																	float PitchMultiplier) {
	SnowRumbleAudio::PlaySoundAtLocation(this, JingleSound, ESnowRumbleAudioMixChannel::Gameplay, Location,
										 JingleVolumeMultiplier, PitchMultiplier, JingleSoundAttenuation);
}
