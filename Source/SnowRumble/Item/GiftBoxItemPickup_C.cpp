...
void AGiftBoxItemPickup::Tick(float DeltaSeconds) {
	...
	const float FloatOffset = FMath::Sin(FloatTimeSeconds * FloatSpeed * UE_TWO_PI) * FloatAmplitude;
	ItemMeshComponent->SetRelativeLocation(InitialMeshRelativeLocation + FVector::UpVector * FloatOffset);
}

void AGiftBoxItemPickup::InitializePickupFromServer(ESnowRumbleGiftItemType NewItemType, FName NewItemId,
...
bool AGiftBoxItemPickup::CanInteractWith(const ASnowRumbleCharacter* Character) const {
	if (!IsValid(Character) || bPickedUp || InteractionRadius <= 0.0f) {
		return false;
	}
...
void AGiftBoxItemPickup::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {
...
void AGiftBoxItemPickup::InitializePlacedPickupFromDefaults() {
	if (ItemType != ESnowRumbleGiftItemType::None || DefaultItemType == ESnowRumbleGiftItemType::None) {
		return;
	}

	ItemType = DefaultItemType;
	ItemId =
		DefaultItemId.IsNone()
			? FName(*StaticEnum<ESnowRumbleGiftItemType>()->GetNameStringByValue(static_cast<int64>(DefaultItemType)))
			: DefaultItemId;
	DisplayName = DefaultDisplayName.IsEmpty() ? GetFallbackDisplayNameForItemType() : DefaultDisplayName;
	OnRep_ItemData();
	ForceNetUpdate();
}
...
FText AGiftBoxItemPickup::GetFallbackDisplayNameForItemType() const {
	...
	return FText::FromString(ItemTypeEnum->GetNameStringByValue(static_cast<int64>(ItemType)));
}

void AGiftBoxItemPickup::NotifyPickedUp(ASnowRumbleCharacter* Character) const {
	...
	const FText EventLogMessage =
		FText::Format(NSLOCTEXT("SnowRumble", "EventLogGiftItemPickedUp", "{0}님이 {1}을 획득했습니다"),
					  FText::FromString(PlayerName), DisplayName);
	const FText PersonalMessage =
		FText::Format(NSLOCTEXT("SnowRumble", "PersonalGiftItemPickedUp", "획득: {0}"), DisplayName);

	for (FConstPlayerControllerIterator It = World->GetPlayerControllerIterator(); It; ++It) {
		ASnowRumblePlayerController* PlayerController = Cast<ASnowRumblePlayerController>(It->Get());
		if (!PlayerController) {
			continue;
		}
...
void AGiftBoxItemPickup::SpawnPickedUpEffect() const {
	...
	UNiagaraFunctionLibrary::SpawnSystemAtLocation(
		this, PickedUpEffect, GetActorLocation() + PickedUpEffectLocationOffset, GetActorRotation());
}

FString AGiftBoxItemPickup::GetCharacterDisplayName(const ASnowRumbleCharacter* Character) const {
	const ASnowRumblePlayerState* SnowRumblePlayerState =
		Character ? Character->GetPlayerState<ASnowRumblePlayerState>() : nullptr;
	if (SnowRumblePlayerState && !SnowRumblePlayerState->GetLobbyPlayerName().IsEmpty()) {
